...
void Engine::getAllActorsInRadius(TCODList<Actor *> &targets, int x, int y, float radius, TargetingSystemBase::TargetType targetType, bool casterHostility) const {
	for (Actor **iterator = engine.actors.begin();iterator != engine.actors.end(); iterator++) {
		Actor *actor = *iterator;
		if (actor->destructible && !actor->destructible->isDead()){
			float distance = actor->getDistance(x,y);
			if ((distance < radius || radius == 0.0) && ((targetType == TargetingSystemBase::NO_TYPE || targetType == TargetingSystemBase::NEUTRAL) || (targetType == TargetingSystemBase::ENEMY && actor->hostile != casterHostility) || (targetType == TargetingSystemBase::FRIENDLY && actor->hostile == casterHostility)) ) {	//just to make sure diagonals are captured
				targets.push(actor);
			}
		}
	}
