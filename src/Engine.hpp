...
public:
	...
	void getAllActorsInRadius(TCODList<Actor *> &targets, int x, int y, float radius = 0.0, SpellCastingConstants::TargetType targetType = SpellCastingConstants::NO_TYPE, bool casterHostility = false) const; //this will add to the passed list all living actors that are within (radius) of the specified tile (x,y), and can discriminate based on TargetType if need be 
