...
public:
	...
	enum SpellEffect {  //this is the effect of the spell (pretty straight-forward)
		...
		DAMAGING_PULL, //21 pulls target(s) strictly towards the caster, while also doing elemental damage to them
		DAMAGING_PUSH, //22 pushes target(s) strictly away from the caster and damages them. perhaps does extra damage if they hit a wall and cannot be pushed the full distance away
		PULL, //23 pull target(s) towards the caster
		PUSH, //24 push target(s) away from the caster
		STRAIGHT_DAMAGE, //25 does HP damage to the target(s)
		BLEED_DAMAGE, //26 does small HP damage to the target(s)
		STAT_DRAIN, //27 reduce the target(s) stat(s)
		STAT_SAPPING, //28 reduce the target(s) stat(s) and increase the caster's corresponding stat(s)
		LIFE_LEECHING, //29 do bleed damage to the target(s) and do a corresponding amount of healing over time to the caster
		MANA_LEECHING, //30 do bleed damage to the target(s)'s mana resource(s) and restore a corresponding amount of the same resource(s) to the caster [do nothing if they have none]
		INSTAKILL, //31 instantly reduce the target(s) HP to zero
		DEBUFFING, //32 lower the stats of a target, or inflicts them with certain other status effects (blinded, slowed, confused, etc) for a certain amount of time
		UNSUMMON,       //33 would seem to be incompatible with most targeting types (or at least it will be usually useless), but it could attempted to completely destroy a summoned target and just weaken non-summoned targets
		BANISHING, //34 send the target(s) to another dimension, removing them from the map, but not necessarily killing them; hopefully, I will get around to making this "other dimension" an accessible location, so that the player could be sent here (or send himself here) and have to fight the enemies he sends away
		MANA_DAMAGE, //35 inflict damage to the target(s) mana resource(s) [do nothing if they have none]
		DOOM_TIMER, //36 set a timer of a set length on the target(s), which decrements each turn; after the timer reaches zero, they are instakilled
		DARKNESS,       //37 not sure how an elemental subtype of a light or darkness spell would work, but perhaps hide or highly increase the stealth value of a creature that would be masked by the elemental darkness (water creature hide in water-darkness)
		PERCENTILE_DAMAGE, //38 deals a set percentage of the target(s) max HP
		RESURRECT, //39 resurrect dead target(s), does nothing to living target(s)
		
		
		//these ones aren't so compatible with all targeting types
		TELEPORTAL, //40 creates a point on the ground to which the caster will return when he casts a teleportation spell, if he's in range; otherwise, his location upon teleportation will be random
		ALTER_TERRAIN, //41 adds or removes hazards from the ground (lava pools, shards of glass, etc)
...
//	int ID; //spellID, unique reference INT to identify a spell based on its attributes, not its name
	...
	virtual bool cast() = 0; //this function resolves the effect of the spell;
};
